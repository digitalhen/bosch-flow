...
# https://github.com/digitalhen/bosch-flow/releases/latest/download/appcast.xml,
...
URL="https://github.com/${REPO}/releases/download/${TAG}/Bosch-Flow-${VER}.zip"
PUBDATE="$(LC_ALL=C date '+%a, %d %b %Y %H:%M:%S %z')"
cat > dist/appcast.xml <<XML
<?xml version="1.0" encoding="utf-8"?>
<rss xmlns:sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle" version="2.0">
  <channel>
    <title>Bosch Flow</title>
    <item>
...
gh release create "$TAG" "$ZIP" dist/appcast.xml \
  --repo "$REPO" --title "Bosch Flow ${VER}" --notes "$NOTES"
